...
NODES_PATH = "final_csv/nodes.csv"
NODE_TAGS_PATH = "final_csv/nodes_tags.csv"
WAYS_PATH = "final_csv/ways.csv"
WAY_NODES_PATH = "final_csv/ways_nodes.csv"
WAY_TAGS_PATH = "final_csv/ways_tags.csv"
...
PINNAMES = ["pincode", "postal_code", "post_code", "postcode"]

MAPPING = {"PO":"Post Office", "Jn":"Junction", "Rd":"Road", "Rd.":"Road", "Rd,":"Road", "Raod":"Road", "St":"Street", "Veedhi":"Street"}
...
def clean_address(addr):
  addr = strip_long_address(addr)
  addr = update_words(addr)
  return addr


# Clean tags
def clean_tag(tag):
  # if key is one of different pincode names, change it and also clean the value of pincode
  if tag['key'] in PINNAMES:
    ...
    if new_pin:
      ...
      tag = {}
      return tag
  # if key is phone or fax clean the value
  if tag['key'] == 'phone' or tag['key'] == 'fax':
    new_val = update_phone_fax(tag['value'])
    if new_val:
      tag['value'] = new_val
    else:
      tag = {}
  ...
  cleaned_addr = clean_address(tag['value'])
  tag['value'] = cleaned_addr
  return tag
    ...
    if element.tag == "way":
      ...
      for child in element:
        if child.tag == "nd":
          ...
          waynode['node_id'] = child.attrib['ref']
          waynode['position'] = node_position
